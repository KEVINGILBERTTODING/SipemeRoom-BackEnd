				...
				foreach ($transaksi as $tr) : ?>
					<tr>
						...
						<td>
							...
							<?php } else { ?>
								<a href="<?= base_url('customer/transaksi/pembayaran/' . $tr->id_sewa); ?>" class="btn btn-sm btn-success">Cek Pemesanan</a>
							<?php } ?>
						</td>
						<td>

							<?php if ($tr->status_sewa == 'Belum Selesai') { ?>
								<a onclick="return confirm('Yakin batal?')" class="btn btn-sm btn-danger" href="<?= base_url('customer/transaksi/batal_transaksi/' . $tr->id_sewa) ?>">Batal</a>
							<?php } else { ?>
